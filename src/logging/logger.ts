...
/** Final file logger settings after env, config, test, and default resolution. */
...
/** Injects a logging config loader for tests and clears cached logger state. */
...
/** Checks whether the file logger would emit the provided level. */
...
/** Returns the process-wide tslog file logger, rebuilding when settings change. */
...
/** Creates a child logger with structured bindings and optional level override. */
...
/** Adapts the tslog logger to the pino-like surface expected by some libraries. */
...
/** Minimal pino-compatible logger contract used by dependency adapters. */
...
/** Resolves current logger settings without creating a logger transport. */
...
/** Resets logger caches, overrides, and test config loaders. */
...
/** Internal logger helpers exposed only for focused tests. */
