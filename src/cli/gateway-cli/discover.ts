...
/** CLI flags accepted by `openclaw gateway discover`. */
export type GatewayDiscoverOpts = {
  timeout?: string;
  json?: boolean;
};

/** Parses discovery timeout flags while preserving the command's fallback timeout. */
...
/** Extracts the connectable host selected from a Bonjour gateway beacon. */
...
/** Extracts the connectable gateway port selected from a Bonjour gateway beacon. */
...
/** Removes duplicate Bonjour beacons while preserving first-seen render order. */
...
/** Renders one gateway beacon as human-readable CLI lines with optional color. */
