...
/** Mocked context-engine compaction result for embedded compaction tests. */
...
/** Mocked global hook runner used around before/after compaction hooks. */
...
/** Mocked runtime plugin loader invoked before hook execution. */
export const ensureRuntimePluginsLoaded: Mock<(params?: unknown) => void> = vi.fn();
/** Mocked context-engine resolver returning a compaction-owning engine. */
export const resolveContextEngineMock = vi.fn(async () => ({
  info: { ownsCompaction: true as boolean },
  compact: contextEngineCompactMock,
}));
/** Mocked model resolver used when compaction rebuilds an embedded session. */
...
/** Mocked session compact implementation for agent-native compaction paths. */
...
/** Mocked internal hook emitter used by compaction flows. */
export const triggerInternalHook: Mock<(event?: unknown) => void> = vi.fn();
/** Mocked replay-history sanitizer for compaction session messages. */
export const sanitizeSessionHistoryMock = vi.fn(
  async (params: { messages: unknown[] }) => params.messages,
);
/** Mocked memory search manager loader used after compaction. */
...
/** Mocked memory search config resolver with post-compaction session sync enabled. */
...
/** Mocked session agent id resolver. */
export const resolveSessionAgentIdMock = vi.fn(() => "main");
/** Mocked resolver for default and session-specific agent ids. */
export const resolveSessionAgentIdsMock = vi.fn(() => ({
  defaultAgentId: "main",
  sessionAgentId: "main",
}));
/** Mocked token estimator used by compaction threshold checks. */
export const estimateTokensMock = vi.fn((_message?: unknown) => 10);
/** Mocked harness policy resolver defaulting to the OpenClaw runtime. */
export const resolveAgentHarnessPolicyMock = vi.fn(() => ({ runtime: "openclaw" }));
/** Mocked context-window resolver for compaction threshold tests. */
...
/** Mutable session message fixture shared by compact-session mocks. */
export const sessionMessages: unknown[] = createDefaultSessionMessages();
/** Mocked session abort-compaction hook. */
...
/** Mocked embedded agent session factory used by compaction tests. */
...
/** Mocked OpenClaw coding tool factory. */
export const createOpenClawCodingToolsMock = vi.fn(() => []);
/** Mocked session manager guard with a flush hook. */
export const guardSessionManagerMock = vi.fn(() => ({
  flushPendingToolResults: vi.fn(),
}));
/** Mocked application of compaction settings from config. */
export const applyAgentCompactionSettingsFromConfigMock = vi.fn();
/** Mocked prepared settings manager factory for embedded agent sessions. */
export const createPreparedEmbeddedAgentSettingsManagerMock = vi.fn(() => ({
  getGlobalSettings: vi.fn(() => ({})),
}));
/** Mocked plugin prompt guidance registry with surface-specific defaults. */
...
/** Resets session-level compaction mocks and mutable message fixtures. */
...
/** Resets all compaction hook harness mocks before loading isolated modules. */
...
/** Installs compaction mocks and imports the compact modules under test. */
