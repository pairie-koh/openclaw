...
/** Minimum spacing floor between heartbeat starts for the same agent. */
...
/** Time window used to detect heartbeat wake feedback loops. */
export const DEFAULT_FLOOD_WINDOW_MS = 60_000;
/** Maximum recent heartbeat starts allowed inside the flood window before deferring. */
export const DEFAULT_FLOOD_THRESHOLD = 5;

/** Cooldown decision returned for one heartbeat wake. */
export type DeferDecision =
  | { defer: false }
  | { defer: true; reason: "not-due" | "min-spacing" | "flood" };

/** Inputs needed to decide whether a heartbeat wake should run now or defer. */
