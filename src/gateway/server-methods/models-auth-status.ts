...
function buildExpiry(
  ...
  const resolvedExpiresAt = asDateTimestampMs(expiresAt);
  if (resolvedExpiresAt === undefined || typeof remainingMs !== "number") {
    return undefined;
  }
  return { at: resolvedExpiresAt, remainingMs, label: formatRemainingShort(remainingMs) };
}
