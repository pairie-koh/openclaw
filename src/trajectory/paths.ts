...
/** Max inline runtime capture stored in a trajectory event. */
export const TRAJECTORY_RUNTIME_CAPTURE_MAX_BYTES = 10 * 1024 * 1024;
/** Max referenced runtime file size copied into an export bundle. */
export const TRAJECTORY_RUNTIME_FILE_MAX_BYTES = 50 * 1024 * 1024;
/** Max serialized runtime event size accepted before truncation/drop handling. */
...
/** Converts arbitrary session ids into bounded filename-safe stems. */
...
/** Builds pointer-file open flags, using `O_NOFOLLOW` when the platform supports it. */
...
/** Resolves the runtime trajectory JSONL path, honoring the contained directory override. */
...
/** Resolves the sidecar pointer path that records the active trajectory file for a session. */
